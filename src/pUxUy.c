...
// don't need for this (newer) version - using unified incrementing
...
// logj(starts with 1), factj (starts with 0) - calculated beforehand
SEXP llik(SEXP Rux, SEXP Ruy, SEXP Rixy, SEXP Riyx, SEXP Rnx, SEXP Rny, 
	   SEXP Rprob, SEXP Rlogj, SEXP Rfactj, SEXP Rr, SEXP Rnm) { 
  int nux, nuy, nuxy, nm, nr, K, nx, ny, ax, ay, bx, by, i, ix, iy, mmax;
  int *ux1, *uy1, *ixy1, *iyx1;  // to not rewrite R obj
  double combx, comby;
  double *p, *logj, *factj, *r, *lik;
  SEXP Rlik;
  ...
  logj  = REAL(Rlogj);
  factj = REAL(Rfactj);
  r     = REAL(Rr);       // grid; matrix with columns for combinations
  nux   = length(Rux);         
  nuy   = length(Ruy);
  nuxy  = length(Rixy);   // length(Rixy) = length(Riyx)
  K     = length(Rprob);
  nr    = length(Rr)/nm;  // the number of r vectors to evaluate over
  ...
  while (vx[0] != bx) {  /* cannot use "<" if ax = 0 */
	...
	sprob[i] = log(sprob[i]);
      ...
      for (i = 0; i < nr; i++) {
        lik[i] += probSxSy(r + i*nm, sprob, nm, mmax); 
      }
    }
  }
